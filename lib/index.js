...
exports.handleHook = function(req, res) {
  if (req.headers['x-github-event'] === 'pull_request') {
    logger.log(req);
    var pr = JSON.parse(req.body.payload);
    ...
    if (merged) {
      scheduleUser(contributor, function (award) {
        if (award) {
          request({
            method: 'POST',
            url: GITHUB_API_URL + 'repos/' + owner + '/' + repo + '/issues/' + number +'/comments',
            json: {
              body: 'Hey! Awesome job! We wish to reward you! ' +
...
exports.auth = function (req, res) {
  awardUser(req.params.user, function(award) {
    if (award) {
      ...
      res.render('index', {message: 'Sorry, you are not in our records or you have been previously awarded. Please inform us is that\'s not the case.'});
    }
  });
};
      ...
      request({
        ...
        awardUser(JSON.parse(b).login, function (award) {
          if (award) {
            ...
            res.render('index', {message: 'It looks like you\'ve already claimed your reward. Please comment on the pull request, if this isn\'t the case.'});
          }
        });
      });
    });
  ...
  awardUser(req.session.user, function(award) {
    if (award) {
      awardedUser(req.session.user, function () {
        ...
        res.render('index', {message: 'Thanks for contributing. Your reward will be delivered shortly! :D'});
      });
    } else {
      res.render('index', {message: 'It looks like you\'ve already claimed your reward. Please comment on the pull request, if this isn\'t the case.'})
    }
...
scheduleUser = function (user, cb) {
  db.get(user, function(e, awarded) {
    if (e !== null) {
      logger.log(e);
      cb(false);
    } else if (awarded === undefined || awarded === null) {
      db.set(user, 'false');
      cb(true);
    } else {
      cb(false);
